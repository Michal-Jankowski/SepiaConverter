...
.code											 ; dyrektywa .code wykorzystywana do inicjalizacji segmentu kodu
												 ...
												 ; ymm0 - ymm8 -> rejestry przechowuj¹ce zawartoœæ sk³¹dowych pikseli tablicy bajtów ( R,G,B,A), znajduj¹ siê tam dane poœrednie oraz sta³e dividier_value, alpha_mask, color_R_mask
												 ; oraz wszystkie dane potrzebne do wykonania programu
												 ; xmm0 -xmm8 -> rejestr pe³ni¹ podobn¹ funkcjê co rejestry ymm0-ymm8 ze wzglêdu na fakt, i¿ wykorzystuje ca³e rejestry ymm, a one zawieraj¹c ca³e rejestry xmm
												 ; xmm9 - xmm10 -> rejestr ³¹duj¹cy wartoœæ przetworzonych pikseli do tablicy spowrotem na odpowiednie miejsce
												 ; r8 -> rejestr przechowuj¹cy wartoœæ wspó³czynnika wype³nienia sepi¹
												 ; r9 -> rejestr przechowuj¹cy wartoœæ wspó³czynnika g³êbii sepii
												 ; rdi -> rejestr do przechowywania informacji o aktualnie ustawionym  elemencie  pêtli
												 ; rcx -> rejestr do przesuwania siê po wskaŸniku na tablice
												 ; rdx -> rejestr przechowujacy informacje o koñcu przedzia³u tablicy
												 ; rsp -> rejestr przetrzymuj¹cy informacje o bazowym wskaŸniku z rejsestru rbp
												 ; al, rax -> przechowywanie informacji o wartoœci z pierwszego piksela w II czêœci programu
												 ; rax, al -> akumulator wykorzystywany do przechowywania wartoœci wspó³czynników intensywnoœci lub g³êbi Sepii wykorzstywany do przetwarzania
												 ; r10,r11,r12,r13,r14 -> parametry poœrednie wykorzystywane w II czêœci programu przy kolorzyzacji
												 ; eax, rbx, r8b -> rejestr wykorzystywany przy operacji wszkuwania funkcjonalnoœci procesora przy CPUID
												 ; cl -> wykorzystywany do ustawienia odpowiedniego bitu funkcjonalnoœci, a nastêpniewykorzystywany przez rejestr eax do logicznego przesuniêcia
												 ;----------------------------------------------------------------------------------------------------------------------
